...
def test_return_something_on_local_repo(local_only_repo: Repo):
    report = get_untracked_report(local_only_repo)
    assert report is not None


def test_return_none_on_tracked_repo(local_repo):
    report = get_untracked_report(local_repo)
    assert report is None


def test_multiple_braches(remote_repo, local_repo):
